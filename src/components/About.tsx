      ...
      <div class='customfont-base'>
        <div class='text-slate-900 my-8 space-y-2'>
          ...
          <p class='text-xl'>
            ...
            numerous enhancements over the original application.
          </p>
        </div>
        ...
        <div class='grid md:grid-cols-4 grid-cols-2 md:grid-rows-2 grid-rows-4 text-slate-900 md:gap-6 gap-4 pb-16 md:px-12'>
          {items.map((item) => (
            <div class={item.color + ' ' + ' place-content-center rounded-2xl md:space-y-2 space-y-2 px-4 md:py-6 py-4 md:w-fit w-full h-full'}>
              <p class='customfont-title md:text-[27px] text-xl font-bold'>{item.name}</p>
